...
from handlers.users.products_all import (python_book, grill, tez_SHIPPING,
                                         oddiy_SHIPPING, joyidan_olib_SHIPPING, basket)
from handlers.users.start import products

from loader import dp, bot

from keyboards.inline.product_key import build_keyboard, build_keyboard_basket
...
async def show_grill_invoices(message: types.Message):
    ...
    await message.answer_photo(photo="https://dostavka-edy-v-istre.ru/wp-content/uploads/2023/04/ab42dcd7d00c435d4ea1e9de43680030.jpg",
                         caption=caption, reply_markup=build_keyboard_basket("grill"))


@dp.callback_query_handler(lambda call: call.data.startswith("basket"))
async def book_basket(call: CallbackQuery):
    ...
    for key in products:
        if key == info:
            ...
            amount = products[key]['prices'][0]['amount']
            # print(amount)
            basket[key] = {'title': title,
                           'price': amount,
                           'currency': currency}
    await call.message.answer(f"Mahsulot: <b>{info}</b> savatingizga qo'shildi")
...
@dp.message_handler(text="GRILL")
async def praktikum_invoice(message: types.Message):
